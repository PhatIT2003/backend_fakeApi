...
const verifyTelegramHash = (req, res, next) => {
  ...
  try {
    ...
    }).map(([key, value]) => `${key}=${value}`).sort() // Đảm bảo thứ tự đúng
    .join('\n');
    ...
    const computedHash = crypto.createHmac('sha256', secretKey).update(dataCheckString).digest('hex'); // Đảm bảo sử dụng 'hex'
    ...
    const member = {
      ...
      token: 100 || null,
      inviter: null,
      // Bạn có thể điều chỉnh logic inviter nếu cần
      address: "0x"
    };
