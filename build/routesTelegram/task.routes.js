...
module.exports = db => {
  ...
  router.post('/complete/:taskId', authMiddleware, (req, res) => {
    try {
      const taskId = req.params.taskId;
      const userId = req.user.id;

      // Kiểm tra nhiệm vụ
      const task = db.get('task').find({
        id: taskId
      }).value();
      if (!task) {
      ...
      const existingUserCollection = db.get('CollectionTask').find({
        userId: userId.toString()
      }).value();

      // Kiểm tra xem task đã tồn tại trong collection của user chưa
      const isTaskAlreadyCompleted = existingUserCollection && existingUserCollection.task.some(t => t.id === taskId);
      ...
      db.get('member').find({
        user_id: userId
      }).update('token', currentToken => (Number(currentToken) || 0) + taskToken).write();
  ...
  router.get('/completed-tasks', authMiddleware, (req, res) => {
    try {
      const userId = req.user.id;
      const userCollection = db.get('CollectionTask').find({
        userId: userId.toString()
      }).value();
