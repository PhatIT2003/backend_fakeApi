...
module.exports = db => {
  ...
  const updateInviteUsage = (inviteCode, newUser) => {
    try {
      ...
      if (inviteIndex !== -1) {
        const hasBeenUsedBefore = invites[inviteIndex].usedBy && invites[inviteIndex].usedBy.some(use => use.userId === newUser.user_id);
    ...
    try {
      ...
      if (existingMemberIndex !== -1) {
        ...
        members[existingMemberIndex] = {
          ...
          inviter: existingMember.inviter === "null" ? inviteCode : existingMember.inviter,
          // Chỉ cập nhật nếu chưa có inviter
