...
module.exports = (db) => {
    ...
    router.post('/complete/:taskId', authMiddleware, (req, res) => {
        try {
            ...
            // Kiểm tra nhiệm vụ
            const task = db.get('task').find({ id: taskId }).value();
            if (!task) {
                return res.status(404).json({ success: false, message: 'Task not found' });
            }
        
            // Kiểm tra xem người dùng đã hoàn thành nhiệm vụ này chưa
            const existingUserCollection = db.get('CollectionTask')
    ...
    router.get('/completed-tasks', authMiddleware, (req, res) => {
        try {
            const userId = req.user.id;
            const userCollection = db.get('CollectionTask')
                .find({ userId: userId.toString() })
                .value();
